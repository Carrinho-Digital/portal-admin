...
const ProdutoListaPromocoes = ({ id }) => {
  ...
  const getDesconto = (promocao) => {
    if (promocao.discountInPercent) {
      ...
      return `R$ ${Number(promocao.discountInPrice).toFixed(2)}`;
    }
  };
  ...
  const renderItems = () => {
    if (!data.length) {
      return (
        <ListGroupItem>
          Não há nenhuma promoção para este produto!{" "}
          <Link to="/promocoes/novo">Crie uma nova</Link>
        </ListGroupItem>
      );
    }

    return data.map((promocao) => (
      <ListGroupItem key={`item-${promocao._id}`}>
